...
| Link protocol logic | portable `sh/net/applink` (JSON serializer + strict `target_w`/`arrival_battery_w`/`mode` parser), firmware only moves bytes | keeps the phone-facing attack/failure surface inside the 100 % desktop-tested gate |
| RANGE mode | on-board open-loop hold of `range_motor_power_w` (remotely tunable, validated ≤ motor max); the app computes the value once from the learned EnergyKnee | full autonomy underway — the efficiency optimum is a slowly-changing property of the hull, not a live stream; ramps, envelope and reserve floor apply unchanged |
| ARRIVAL mode | phone-streamed battery-power budget tracked closed-loop; entry needs a fresh budget, and >10 s of silence degrades to SOLAR with fault bit 13 (`arrival-stale`) | "phone computes, ESP executes": forecast and route math stay off the MCU, while the boat never depends on the phone for safety — same staleness doctrine as REMOTE |
